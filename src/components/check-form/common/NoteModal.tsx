...
export default function NoteModal({ open, title = "แก้ไขหมายเหตุ", value, onChange, onCancel, onSave }: Props) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40">
      <div className="w-full max-w-xl sm:max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b">
          ...
          <button onClick={onCancel} className="rounded-md p-1 text-gray-500 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-300">×</button>
        </div>
