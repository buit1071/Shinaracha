...
export default function Form8_1({ jobId, equipment_id, name, onBack }: Props) {
  ...
  const Section = ({ id, title, children }: { id: string; title: string; children: React.ReactNode }) => {
    const SECTION_TITLES_TH: Record<string, string> = {
      report: "ส่วนรายงานผลการตรวจ (ข้อมูลทั่วไป + เอกสารประกอบ)",
      general: "1.1 ข้อมูลทั่วไป (General)",
      info: "1.2 ผัง/แผนที่",
      inspect: "1.3 รายละเอียดตรวจ 7 ข้อ",
      summary: "1.4 สรุป/ลงนาม",
      "plan-2-1": "2.1 ขอบเขตงาน",
      "plan-2-2": "2.2 แนวทางการตรวจ",
      "plan-2-3": "2.3 รายละเอียดตรวจ",
      "plan-2-4": "2.4 มาตรฐาน/อ้างอิง",
      "plan-2-5": "2.5 ความถี่",
      "plan-2-6": "2.6 ผลการตรวจ",
      "plan-2-7": "2.7 สรุป/ลงนาม",
    };
    const displayTitle = SECTION_TITLES_TH[id] ?? title;
    return (
      <section className="w-full mb-3">
        <button type="button" onClick={() => toggle(id)} aria-expanded={openSections.includes(id)} className="w-full grid h-[5vh] select-none cursor-pointer">
  ...
  return (
    <div className="p-2 no-anchor">
      ...
      <div className="w-full flex justify-end mb-2">
        ...
        <button
          type="button"
          onClick={() => exportToPptxForm8_1(formData ?? null, formData.dbId ?? null, jobId ?? "")}
      ...
      <div className="w-full shadow-sm border bg-white text-black p-2">
        {/* พับใหญ่: 1) รายงานผลการตรวจสอบป้าย */}
        <section className="border-2 border-gray-200 rounded-2xl overflow-hidden bg-white mb-5">
          <button type="button" className="w-full flex items-center justify-between px-4 py-3 bg-gray-100 hover:bg-gray-200" onClick={() => toggleMain("main-1")}>
            <span className="font-semibold text-gray-800">1) รายงานผลการตรวจ</span>
          ...
          <div className={`grid transition-[grid-template-rows,opacity] duration-300 ease-out ${openMain.includes("main-1") ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
            <div className="overflow-hidden"><div className="p-2">
              <Section id="report" title="รายงานผลการตรวจสอบป้าย (หัวเรื่อง + ข้อมูลหลัก)">
                <Form8_1_Report_1_0 value={formData.report} onChange={setReport} jobId={jobId} name={name} inspectorName={username} />
              </Section>
              <Section id="general" title="1.1 ส่วนที่ 1 ขอบเขตงาน (General)">
                <Form8_1_Report_1_1 value={formData.report} onChange={setReport} />
              </Section>
              <Section id="info" title="1.2 ส่วนที่ 2 ข้อมูลทั่วไป/ผัง-แผนที่">
                <Form8_1_Report_1_2 general={formData.general} location={formData.location} onChangeGeneral={setGeneral} onChangeLocation={setLocation} photos={formData.photos} onChangePhotos={setPhotos} />
              </Section>
              <Section id="inspect" title="1.3 ส่วนที่ 3 รายละเอียดการตรวจ 7 หัวข้อ">
  ...
  <Main1_Inspect installType={formData.report?.installType} value={formData.inspect} onChange={setInspect} />
              ...
              <Section id="summary" title="1.4 ส่วนที่ 4 สรุปผล/ลงนาม">
        ...
        {/* พับใหญ่: 2) แผนปฏิบัติการการตรวจบำรุงรักษาป้ายฯ */}
        <section className="border-2 border-gray-200 rounded-2xl overflow-hidden bg-white">
          <button type="button" className="w-full flex items-center justify-between px-4 py-3 bg-gray-100 hover:bg-gray-200" onClick={() => toggleMain("main-2")}>
            <span className="font-semibold text-gray-800">2) แผนการบำรุงรักษา/ตรวจตามมาตรฐาน</span>
            <span className="text-sm text-gray-500" />
          </button>
          <div className={`grid transition-[grid-template-rows,opacity] duration-300 ease-out ${openMain.includes("main-2") ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}>
            <div className="overflow-hidden"><div className="p-2">
              <Section id="plan-2-1" title="2.1 ส่วนที่ 1 ขอบเขตงาน">
                <Form8_1_Plan_2_1 value={formData.plan} />
              </Section>
              <Section id="plan-2-2" title="2.2 ส่วนที่ 2 คำอธิบายแผน">
                <Form8_1_Plan_2_2 value={formData.plan} />
              </Section>
              <Section id="plan-2-3" title="2.3 ส่วนที่ 3 รายละเอียด">
                <Form8_1_Plan_2_3 value={formData.plan} />
              </Section>
              <Section id="plan-2-4" title="2.4 ส่วนที่ 4 แนวทาง/มาตรฐาน">
                <Form8_1_Plan_2_4 value={formData.plan} />
              </Section>
              <Section id="plan-2-5" title="2.5 ส่วนที่ 5 ความถี่">
                <Form8_1_Plan_2_5 value={formData.plan} onChange={setPlan} />
              </Section>
              <Section id="plan-2-6" title="2.6 ส่วนที่ 6 ผลการตรวจ">
                <Form8_1_Plan_2_6 value={formData.plan} onChange={setPlan} onOpenDefect={openDefectDialog} />
              </Section>
              <Section id="plan-2-7" title="2.7 ส่วนที่ 7 สรุป/ลงนาม">
