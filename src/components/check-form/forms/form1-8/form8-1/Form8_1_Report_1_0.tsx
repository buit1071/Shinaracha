...
export default function Main1_Report({ value, onChange, jobId, name, inspectorName }: Props) {
  ...
  const pickMainImage = (file: File | null) => {
    ...
    onChange({ headerImageUrl: filename });
    // อัปโหลดทันทีแบบไม่บล็อก และสลับ preview เป็น URL ระยะไกลเมื่อสำเร็จ
    void (async () => {
      try {
        const ok = await uploadIfNeeded(url, filename);
        if (ok) setMainPreviewUrl(buildRemoteUrl(filename));
      } catch { /* ignore; ไม่มีผลกับการบันทึก เพราะเราบันทึก filename อยู่แล้ว */ }
    })();
  };
