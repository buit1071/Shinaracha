...
export default function Plan2_7_Summary({ value, onChange }: Props) {
  ...
  const summarizeRound = React.useCallback((res: PlanUsability | undefined, key: string, round: 1 | 2 | 3): UsabilityStatus | undefined => {
    ...
    let hasStatus = false;
    for (const r of rows) {
      const status = (round === 1 ? r.statusRound1 ?? r.status : round === 2 ? r.statusRound2 : r.statusRound3) as UsabilityStatus | undefined;
      if (!status) continue;
      hasStatus = true;
  ...
  const rows: SummaryRow[] = React.useMemo(() => {
    ...
    for (const g of SUMMARY_GROUPS) {
      ...
      const statusRound3 = summarizeRound(plan.usabilityPlan as any, g.key as string, 3);
      ...
      out.push({
        ...
        statusRound3: base.statusRound3 ?? statusRound3,
      } as SummaryRow);
    }
    return out;
  ...
  const renderRow = (r: SummaryRow, i: number) => {
    const toggle = (round: 1 | 2 | 3, st: UsabilityStatus) => {
      ...
      const curr = (round === 1 ? r.statusRound1 ?? (r as any).status : round === 2 ? r.statusRound2 : r.statusRound3) as UsabilityStatus | undefined;
    ...
    return (
      <tr key={`row-${i}`} className="odd:bg-white even:bg-gray-50">
        ...
        <td className="border px-2 py-2 text-center">
          <CheckTick size="sm" className="m-0" checked={r.statusRound3 === "usable"} onChange={() => toggle(3, "usable")} />
        </td>
        <td className="border px-2 py-2 text-center">
          <CheckTick size="sm" className="m-0" checked={r.statusRound3 === "unusable"} onChange={() => toggle(3, "unusable")} />
        </td>
        <td className="border px-2 py-2 text-center">
