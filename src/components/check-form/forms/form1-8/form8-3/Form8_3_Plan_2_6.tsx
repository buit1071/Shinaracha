...
export default function Plan2_6_Results({ value, onChange, onOpenDefect }: Props) {
  ...
  const setRow = (
    ...
    idx: number,
    patch: Partial<Pick<UsabilityRow, "status" | "statusRound1" | "statusRound2" | "statusRound3" | "note" | "defects">>
  ) => {
  ...
  const Row = ({ group, r, i }: { group: "structural" | "electrical" | "lightning" | "others"; r: UsabilityRow; i: number }) => {
    const toggle = (round: 1 | 2 | 3, st: UsabilityStatus) => {
      const key = round === 1 ? "statusRound1" : round === 2 ? "statusRound2" : "statusRound3";
      const curr = (round === 1 ? r.statusRound1 ?? r.status : round === 2 ? r.statusRound2 : r.statusRound3) as UsabilityStatus | undefined;
    ...
    const round3 = r.statusRound3;
    const defectDisabled = round1 !== "unusable" && round2 !== "unusable" && round3 !== "unusable";
    return (
      <tr className="odd:bg-white even:bg-gray-50">
        ...
        <td className="w-24 border px-2 py-2 text-center h-10 align-middle">
          <CheckTick checked={round3 === "usable"} onChange={() => toggle(3, "usable")} />
        </td>
        <td className="w-24 border px-2 py-2 text-center h-10 align-middle">
          <CheckTick checked={round3 === "unusable"} onChange={() => toggle(3, "unusable")} />
        </td>
  ...
  return (
    <section className="space-y-4">
        ...
        <table className="min-w-full text-sm border tbl-strong vhead-compact table-fixed">
          <thead>
            <tr className="bg-gray-100 text-gray-700">
              ...
              <th className="border px-2 py-2 text-center" colSpan={2}>รอบที่ 3</th>
          ...
          <tbody>
            <tr>
              <td className="border px-2 py-2 bg-gray-100 font-medium" colSpan={10}>โครงสร้าง</td>
            </tr>
            ...
            <tr>
              <td className="border px-2 py-2 bg-gray-100 font-medium" colSpan={10}>ระบบไฟฟ้า</td>
            </tr>
            ...
            <tr>
              <td className="border px-2 py-2 bg-gray-100 font-medium" colSpan={10}>ระบบป้องกันฟ้าผ่า</td>
            </tr>
            ...
            <tr>
              <td className="border px-2 py-2 bg-gray-100 font-medium" colSpan={10}>ระบบ/อุปกรณ์อื่น ๆ</td>
            </tr>
      ...
      {/* DefectPopup อยู่รวมกับ Form8_1 */}
    </section>
  );
}
